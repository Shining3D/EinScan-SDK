...
void GMatrix<Real>::Allocate (bool bSetToZero)
{
    ...
    m_afData = SN3D_NEW Real[m_iQuantity];
    if (bSetToZero)
    {
        memset(m_afData,0,m_iQuantity*sizeof(Real));
    }

    m_aafEntry = SN3D_NEW Real*[m_iRows];
...
{
    SN3D_DELETE[] m_afData;
    SN3D_DELETE[] m_aafEntry;
}
//----------------------------------------------------------------------------
template <class Real>
...
bool GMatrix<Real>::GetInverse (GMatrix<Real>& rkInverse) const
{
    ...
    int* aiColIndex = SN3D_NEW int[iSize];
    int* aiRowIndex = SN3D_NEW int[iSize];
    bool* abPivoted = SN3D_NEW bool[iSize];
    ...
    for (int i0 = 0; i0 < iSize; i0++)
    {
        ...
        if (fMax == (Real)0.0)
        {
            ...
            SN3D_DELETE[] aiColIndex;
            SN3D_DELETE[] aiRowIndex;
            SN3D_DELETE[] abPivoted;
            return false;
        }
